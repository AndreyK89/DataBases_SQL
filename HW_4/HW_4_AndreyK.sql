...
-- Задача 2. Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.
/*
...
CREATE TABLE users (
    ...
    lastname VARCHAR(50) COMMENT 'Фамилия', -- Тест
...
-- 2. Определить кто больше поставил лайков (всего): мужчины или женщины.  
 ...
 SELECT CASE (gender)
        WHEN 'm' THEN 'мужчины'
        WHEN 'f' THEN 'женщины'
    	END AS 'Кго больше', COUNT(*) as 'лайки'
FROM profiles p 
JOIN likes l 
...
SELECT CONCAT(firstname, ' ', lastname) AS 'Макс. сообщений юзеру id=1 отправил:' 
FROM users u
JOIN
messages m
